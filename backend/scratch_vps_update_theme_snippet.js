...
async function main() {
    ...
    // Read the local updated liquid template
    const liquidPath = '/root/gemini-app/shopify-liquid-templates/make-an-offer.liquid';
    ...
    // Upload snippet
    console.log(`Uploading snippets/make-an-offer.liquid to active theme ${activeTheme.id}...`);
    const assetUrl = `https://${shop.domain}/admin/api/2024-01/themes/${activeTheme.id}/assets.json`;
    const assetPayload = {
        asset: {
            key: 'snippets/make-an-offer.liquid',
            value: liquidContent
        }
    };
    ...
    if (uploadRes.data && uploadRes.data.asset) {
        console.log(`✅ Successfully updated snippets/make-an-offer.liquid on live Shopify theme!`);
    } else {
